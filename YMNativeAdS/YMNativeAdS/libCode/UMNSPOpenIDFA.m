...
@implementation UMNSPOpenIDFA

+ (NSString*) sameDayOpenIDFA
{
    return [(NSArray*)[UMNSPOpenIDFA threeDaysOpenIDFAArray] objectAtIndex:0];
}

+ (BOOL)OIFAcanOpenURL:(NSURL *)url{
...
+ (NSString*) threeDaysOpenIDFAArray {
    ...
    //
    [_s_appmap appendString:[self OIFAcanOpenURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@%@%@",_f,_b,_c,_s,_s]]]?@"|":@"-"];
    for (id baseid in base) {
        [_s_appmap appendString:[self OIFAcanOpenURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@%@%@%@",_f,_b,[baseid stringValue],_c,_s,_s]]]?@"|":@"-"];
    }
...
#pragma clang diagnostic ignored "-Wunused-value"
 ...
 #pragma clang diagnostic pop
    ...
    return [openIDFAs componentsJoinedByString:@"|"];
}
